...
for index__ in range(10):
    ...
    for batch_i in range(0, num_samples, BATCH_SIZE):
        ...
        obs = all_pov_obs[batch_indices]
        l_a = all_last_action[batch_indices]
        # l_a.shape
...
all = []
for av in action_obs:
    d1 = np.sum((all_a - av)**2, axis=1)
    all.append(np.argmin(d1))

a = np.unique(all, return_counts=True)
len(a[0])

for counter in range(1, 123):
    # counter = 2
    print("---1---")
    a = all_a[counter]
    d_1 = np.all((a == action_obs) , axis=1)
    index = np.where(d_1)[0][0]
    c1_main = 0
    c2_main = 0
    ...
    for k in key_as_vetor.keys():
        if key_as_vetor[k][index] != 0:
            ...
            if k in key_as:
                v = key_as_v[k][v]
            print(k, v)
        c1_main = key_as_vetor['c1'][index]
        c2_main = key_as_vetor['c2'][index]

    a = all_a[counter]
    index = np.where(np.array(all) == counter)[0]
    stack_key = []
    c1 = []
    c2 = []
            ...
            if k not in ['c1', 'c2']:
                ...
                if key_as_vetor[k][i] != 0:
                    ...
                    if k in key_as:
                        ...
                        ak += k + ' '
                    stack_key.append(ak)
        c1.append(key_as_vetor['c1'][i])
        c2.append(key_as_vetor['c2'][i])

    print(np.unique(stack_key))
    # x = np.abs(c1) + np.abs(c2)
    #
    # x = np.abs(c1-c1_main)
    # print(np.std(x))
    # # print(np.sum(x <= 10)/len(c1))
    #
    # x = np.abs(c2 - c2_main)
    # np.sum(x <= 10)/len(c1)
    x = np.abs(c1) + np.abs(c2) - (np.abs(c1_main) + np.abs(c2_main))
