...
class NatureCNN(nn.Module):
    ...
    def __init__(self, input_shape, output_dim, data_size = 20):
        super().__init__()
...
# print(np.bincount(all_actions[:,1]))
for index__ in range(10):
    ...
    index_1 = index_1[:int(len(index_2))]
    print(len(index_1))
    index_1_1 = np.where((tmp == a2_0_Index + 1))[0]
    np.random.shuffle(index_1_1)
    index_1_1 = index_1_1[:int(len(index_2))]
    ...
    for batch_i in range(0, num_samples, BATCH_SIZE):
        ...
        l_a = np.zeros((len(batch_indices), all_last_action.shape[1]), dtype=np.float32)
        inven_0 = np.zeros((len(batch_indices),  all_data_obs[0].shape[1]), dtype=np.float32)

        for j in range(len(batch_indices)):
            index = batch_indices[j]
            obs[j] = all_pov_obs[index]
            l_a[j] = all_last_action[index]
            inven_0[j] = all_data_obs[0][index]
        # inven_0.shape
...
th.save(network.state_dict(), '1_non_rnn.pth')
th.save(network_1.state_dict(), '2_non_rnn.pth')
th.save(network_2.state_dict(), '3_non_rnn_tmp.pth')
th.save(network_3.state_dict(), '4_non_rnn.pth')
...
network.load_state_dict(th.load('1_non_rnn.pth'))

network_1 = NatureCNN((max_shape, 64, 64), len(list_key[1]), 20).cuda()
network_1.load_state_dict(th.load('2_non_rnn.pth'))

network_2 = NatureCNN((max_shape, 64, 64), len(list_key[2]), 20).cuda()
network_2.load_state_dict(th.load('3_non_rnn.pth'))

network_3 = NatureCNN((max_shape, 64, 64), len(list_key[3]), 20).cuda()
network_3.load_state_dict(th.load('4_non_rnn.pth'))
...
for episode in range(50):
    ...
    for i in range(18000):
        ...
        inven = np.concatenate((inven_m, l_a[0]), axis=-1)
        ...
        new_a = tranfer_action(new_a, new_a['camera'][0], new_a['camera'][1])
