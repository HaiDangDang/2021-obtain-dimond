...
def on_press(key):
    ...
    try:
        ...
        if key_on_press == 'v' and nearbyCraft:
            ...
            nearbyCraft = False
            print(nearbyCraft_l[nearbyCraft_i])
            last_ = 'nearbyCraft'
        ...
        #     place = False
        #     last_ = 'place'
        if key_on_press == 'z' and equip:
            equip = False
            if not lock:
                action['equip'] = equip_l[equip_i]
        ...
        if key_on_press == '1':
            ...
            if carft_index == len(carft_list):
                carft_index = 0
            print('craft', carft_list[carft_index])
        if key_on_press == '2':
            nearbyCraft_i += 1
            if nearbyCraft_i == len(nearbyCraft_l):
                nearbyCraft_i = 0
            print('nearbyCraft_l', nearbyCraft_l[nearbyCraft_i])
        if key_on_press == '3':
...
# len(history['action'])
#
# with open('history_71.pkl', 'wb') as f:
...
network_3 = NatureCNN_5((6, 64, 64), len(next_items_list)).cuda()
network_3.load_state_dict(th.load('copy_image.pth'))
index = 1
action = OrderedDict([('attack', 0),
             ('back', 0),
             ('camera', [0,0]),
             ('craft', 'none'),
             ('equip', 'none'),
...
while (True):
    ...
    if pasue:
        ...
        x,_ = dataset_action_batch_to_actions(action)

        e = env.action_space.noop()

        e['vector'] = action_vector_all[x][0]
        check = False
        if x in none_move_action:
            if x != 'attack':
                check = True
        ...
        # invntory_change = np.sum(last_inventory - obs['vector']) != 0
        craft_success = False
        if len(last_craft) != 0:
            first_craft = last_craft[0]
            first_time = counter_craft[0]
            if first_time + 3 == time_add:
                if np.sum(obs['vector'] - last_obs):
        ...
        if np.sum(obs['vector'] - last_obs) != 0 and not craft_success:
            counter_ex += 1
            ...
            # p_2 = th.softmax(network_3(pov_items), dim=1)[0]
            # p_2 = p_2.detach().cpu().numpy()
            error_distance = np.sum(np.sqrt((KNN - (obs['vector'] - last_obs))**2), axis=1)

            # print("exchange", next_items_list[y_mask[np.argmin(error_distance)]])
            y_mask.shape
            # break
            # print("exchange", counter_ex, next_items_list[np.argmax(p_2)])
        previous = obs['pov']
